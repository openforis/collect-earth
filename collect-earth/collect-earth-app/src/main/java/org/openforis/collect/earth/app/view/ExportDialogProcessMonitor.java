...
public class ExportDialogProcessMonitor extends Thread {
	...
	public ExportDialogProcessMonitor(AbstractProcess<Void, DataExportStatus> exportProcess, JFrame parentFrame,  RecordsToExport recordsToExport, DataFormat exportType, EarthSurveyService earthSurveyService, LocalPropertiesService localPropertiesService ) {
		super();
	...
	public void run() {
		...
		try {
			...
			if( exportProcess.getStatus().isComplete() && exportType.equals( DataFormat.ZIP_WITH_XML ) && recordsToExport.equals(RecordsToExport.MODIFIED_SINCE_LAST_EXPORT) ){
